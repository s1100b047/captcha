...
public partial class answerimage3 : System.Web.UI.Page
{
    private string Answer1()
    {
        string digital3 = String.Empty; ;
        digital3 = Session["square"].ToString();
        Session["digital3"] = digital3;
        return digital3;
    }
    private string Answer2()
    {
        string count3 = String.Empty; ;
        count3 = "2";
        Session["count3"] = count3.ToString();
        return count3;
    }
    private void shapeImage(string color)//將隨機圖形生成圖片
    {
        ...
        string shapenumber3 = String.Empty;
        shapenumber3 = "13";
        g.DrawRectangle(pen, 30, 30, 30, 30);//正方型
        Brush blueBrush = new SolidBrush(Color.Blue);
        Session["shapenumber3"] = shapenumber3.ToString();
    ...
    private void colorImage(string color)//將亂數數字生成圖片
    {
        ...
        Graphics g = Graphics.FromImage(image);
        Color rndcolor;
        string colornumber3 = String.Empty;
        Session["colornumber3"] = "23";
        rndcolor = Color.Gray;
        g.Clear(rndcolor);
        //Graphics.FromImage從指定的 Image 創建新的 Graphics。
        //Graphics 類 封裝一個 GDI+ 繪圖圖面。此類不能被繼承。Graphics 類提供將物件繪製到顯示裝置的方法。
