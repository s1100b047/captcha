...
public partial class createcode : System.Web.UI.Page
{
    private string RndNum() //隨機生成驗證碼
    {
        int number;
        char code;
        string checkCode = String.Empty; //表示空字串。 此欄位為唯讀。
        System.Random random = new Random(Guid.NewGuid().GetHashCode()); //表示偽亂數產生器，一種能夠產生滿足某些隨機性統計要求的數位序列的設備。
        for (int i = 0; i < 6; i++)
        {
            number = random.Next(); //返回非負亂數。
            if (number % 2 == 0)
    ...
    private void CreateCheckCodeImage(string checkCode)//將驗證碼生成圖片
    {
        if (checkCode == null || checkCode.Trim() == String.Empty)
            return;
        System.Drawing.Bitmap image = new System.Drawing.Bitmap((int)Math.Ceiling(checkCode.Length * 20.0), 35);
        ...
        try
        {
            ...
            Font font = new System.Drawing.Font("Comic Sans MS", 15, (System.Drawing.FontStyle.Bold | System.Drawing.FontStyle.Italic));
